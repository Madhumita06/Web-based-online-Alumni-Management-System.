...
Statement st=con.createStatement();
un="user"+i;
String sql="insert into faculty(id,name,lname,address,city,state,zip,phone,email,position,company,pwd) values('"+i+"','"+name+"','"+lname+"','"+addr+"','"+city+"','"+state+"','"+zip+"','"+tele+"','"+email+"','"+posi+"', '"+com+"', '"+pwd+"')";
...
if(q>0)
{
	response.sendRedirect("profilepic3.jsp?id="+email+"");

	//response.sendRedirect("register.jsp?message=succ");
}
else
	response.sendRedirect("signup3.jsp?message=fail");
